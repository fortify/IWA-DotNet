    ...
    public class SeedData
    {
        public static async void Initialize(IServiceProvider serviceProvider)
        {
                    ...
                    DbContextOptions<ApplicationDbContext>>()))
            {
                ...
                {
                    logger.LogError("Role Data has already been seeded!");
                }
                else
                {
                    SeedRoles(serviceProvider.GetRequiredService<RoleManager<IdentityRole>>());
                }

                // Look for any users.
                if (context.Users.Any())
                {
                    logger.LogError("Users Data has already been seeded!");
                } 
                else
                {
        ...
        private static void SeedUsers(UserManager<ApplicationUser> userManager)
        {
            ...
            if (userManager.FindByEmailAsync("user@localhost.com").Result == null)
            {
                User user = new User();
